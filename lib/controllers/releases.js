const { releaseAndDeployPixSite, releaseAndDeployPixBotTest } = require('../services/releases');
...
module.exports = {
  ...
  async createAndDeployPixSiteRelease(request, h) {
    ...
    await releaseAndDeployPixSite(releaseType);
    return true;
  },

  async createAndDeployPixTestRelease(request, h) {
    if (!request.payload || !request.payload.authorizationToken) {
      return h.response().code(401);
    }
    ...
    await releaseAndDeployPixBotTest(releaseType);
    return true;
  },
