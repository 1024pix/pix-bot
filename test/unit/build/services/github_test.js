...
describe('Unit | Build | github-test', function () {
  ...
  describe('Github request caching', function () {
    it('includes If-None-Match header from Etag previously received', async function () {
      // given
      const username = 'Gerrome';

      const etag = 'response-etag';

      const checkNock1stCall = nock('https://api.github.com')
        .get(`/orgs/1024pix/members/${username}`)
        .reply(204, null, { etag });

      const checkNock2ndCall = nock('https://api.github.com', {
        headers: {
          'If-None-Match': etag,
        },
      })
      ...
      // when
      await githubService.checkUserBelongsToPix(username);
      const belongsToPix = await githubService.checkUserBelongsToPix(username);

      // then
      expect(checkNock1stCall.isDone()).to.be.true;
      expect(checkNock2ndCall.isDone()).to.be.true;
      expect(belongsToPix).to.be.true;
    });
  });
